...
#' @name findBasePairsRNAcoFold
#' @docType methods
#' @rdname findBasePairsRNAcoFold
#'
#' @export
findBasePairsRNAcoFold = function(startPos1,
                                  endPos1,
                                  seq1,
                                  startPos2,
                                  endPos2,
  ...
  vienna = sub(" .*", "", x[3])
  startl = as.numeric(gsub(
    ".* (\\d{1,3}),\\d{1,3}\\s+:\\s+(\\d{1,3}),\\d{1,3} .*",
    "\\1",
    x[3],
    perl = T
  ))
  endl = as.numeric(gsub(
    ".* \\d{1,3},(\\d{1,3})\\s+:\\s+\\d{1,3},(\\d{1,3}) .*",
    "\\1",
    x[3],
    perl = T
  ))
  startr = as.numeric(gsub(
    ".* (\\d{1,3}),\\d{1,3}\\s+:\\s+\\d{1,3},(\\d{1,3}) .*",
    "\\2",
    x[3],
    perl = T
  ))
  endr = as.numeric(gsub(
    ".* \\d{1,3},(\\d{1,3})\\s+:\\s+(\\d{1,3}),\\d{1,3} .*",
    "\\2",
    x[3],
    perl = T
  ...
  for (i in 1:nchar(vienna)) {
    ...
    k = i + additionr
    l = substr(vienna, j, j)
    r = substr(vienna, (nchar(vienna) + 1) - k, (nchar(vienna) + 1) -
                 k)
    #print(paste(j,k,l,r))
